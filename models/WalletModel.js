...
export class WalletModel {
  static async createUserWallet(userId, currency = 'BRL') {
  ...
  static async getUserWallet(userId, currency = 'BRL') {
    const { rows } = await pool.query(
      `
      SELECT *
      FROM wallets
  ...
  static async getHouseWalletByUserId(userId, currency = 'BRL') {
    const { rows } = await pool.query(
      `
      SELECT *
      FROM wallets
  ...
  static async createHouseWallet(userId, currency = 'BRL') {
  ...
  static async getOrCreateHouseWallet(userId, currency = 'BRL') {
    const existing = await this.getHouseWalletByUserId(userId, currency)
    if (existing) return existing
    return await this.createHouseWallet(userId, currency)
  }

  static async credit(walletId, { amount, description, meta, externalId }) {
  ...
  static async debit(walletId, { direction = 'DEBIT', amount, description, meta, externalId }) {
    const wallet = await this.getById(walletId)
    if (!wallet) throw new Error('Wallet not found')
  ...
  static async mutateBalance(walletId, { amount, description, meta, externalId }) {
    const wallet = await this.getById(walletId)
    if (!wallet) throw new Error('Wallet not found')
